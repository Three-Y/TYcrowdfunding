 ...
 <div class="navbar-wrapper">
      <div class="container">
            ...
            <div id="navbar" class="navbar-collapse collapse" style="float:right;">
              <jsp:include page="/WEB-INF/jsp/common/memberinfo.jsp"></jsp:include>
            </div>
			  </div>
			</nav>
    ...
    <div class="container theme-showcase" role="main">
      <div class="page-header">
		...
		<form id="certForm" role="form" method="post" enctype="multipart/form-data" style="margin-top:20px;">
			<c:forEach items="${sessionScope.certList }" var="cert" varStatus="status">
			  <div class="form-group">
				<label for="exampleInputEmail1">${cert.name }</label>
				<input type="text" name="memberCerts[${status.index }].certid" value="${cert.id }" style="display : none">
				<input type="file" name="memberCerts[${status.index }].imgFile" class="form-control" style="height:40px">
	            <br>
        ...
        $("#nextBtn").click(function(){
        	var options = {
            		...
            		beforeSubmit : function(){//注意不是beforeSend
            			loadingIndex = layer.msg('提交数据中', {icon: 16});
            		},
            		success : function(result){
            			layer.close(loadingIndex);
            			if(result.success){
            				layer.msg(result.message, {time:1000, icon:6, shift:6});
            				window.location.href="${APP_PATH}/member/checkMail.htm"
            			}else{
