...
import yan.ibbie.TYcrowdfunding.util.DesUtil;
...
public class SpringJavaMailTest {
	public static void main(String[] args) throws Exception {
		...
		javaMailSender.setDefaultEncoding("UTF-8");//设置内容编码
		
		MimeMessage mail = javaMailSender.createMimeMessage();//一份情书
		MimeMessageHelper helper = new MimeMessageHelper(mail);
		helper.setSubject("告白书"); //邮件标题
		
		StringBuilder content = new StringBuilder();//用于编写内容

		String param = "sb";
		param = DesUtil.encrypt(param, "abcdefghijklmnopquvwxyz");//加密

		content.append("<a href='http://www.atcrowdfunding.com/test/act.do?p=" + param + "'>激活链接</a>");
		
		helper.setText(content.toString(), true);
		
		helper.setFrom("admin@yan.com");//发送者
		helper.setTo("test@yan.com");//接收者
